...
public class BhagaChall{
		 ...
		 */
		public int moveTigre(int id, int tigre, int direcao){
			tigre = tigre+1;
                        // se ainda não tem partida
			if(temPartida(id) == 0){
				return -2;
			...
			ArrayList<Integer[]> comerCabras = tigreTemQueComer();
                        if(comerCabras.size() > 0){
                            for(Integer[] tigredirecao : comerCabras){
                                if(tigredirecao[0] == tigre && tigredirecao[1] == direcao){
                                    int[] posicaoTigre = tabuleiro.findTigre(tigre+48);
                                    int[] movimentoTigreSemPulo = movesSemPulo[posicaoTigre[0]][posicaoTigre[1]].getDirecao(direcao);
                                    int[] movimentoTigreComPulo = movesComPulo[posicaoTigre[0]][posicaoTigre[1]].getDirecao(direcao);
		 ...
		 */	
		public int moveCabra(int id, int cabra, int direcao){
			// se ainda não tem partida
			if(temPartida(id) == 0){
				return -2;
